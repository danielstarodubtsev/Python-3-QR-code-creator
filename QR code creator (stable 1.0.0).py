    ...
    while any([None in i for i in qr]):
        ...
        if qr[y][x] is None:
            if data:
                ...
                bit = 0 # TODO: for some reason these bits aren't the same as the ones that qrcoder.ru gives
                        # QR is still decodable due to correction bytes, but it'd be better to fix this
