...
class InvalidCorrectionLevelException(ValueError):
    pass
...
def get_correction_level():
    ...
    if correction_level not in {'L', 'M', 'Q', 'H', 'LOW', 'MEDIUM', 'QUARTILE', 'HIGH'}:
        raise InvalidCorrectionLevelException('Invalid correction level input')
...
def add_mask_and_correction_level_codes(qr, version, correction_level, mask_number):
    qr = deepcopy(qr)
    size = len(qr[0])
...
def create_QR_code(resulting_data, version, correction_level, mask_number):
    '''
    ...
    qr = add_mask_and_correction_level_codes(qr, version, correction_level, mask_number)
    qr = fill_qr_with_data(qr, resulting_data, mask)
...
def score_rule_1_vertical(qr):
    # It's easier to simply transpose and apply previous function
    return score_rule_1_horizontal([list(line) for line in array(qr).transpose()])
...
def score_rule_3_vertical(qr):
    # It's easier to simply transpose and apply previous function
    return score_rule_3_horizontal([list(line) for line in array(qr).transpose()])
...
def qr_score(qr):
    ...
    scores.append(score_rule_1_horizontal(qr))
    scores.append(score_rule_1_vertical(qr))
    scores.append(score_rule_2(qr))
    scores.append(score_rule_3_horizontal(qr))
    scores.append(score_rule_3_vertical(qr))
    scores.append(score_rule_4(qr))

    return sum(scores)


def create_qr_with_best_mask(data, version, correction_level, mask=None):
    all_qr = []
    for mask_number in range(8):
        QR_code = create_QR_code(data, version, correction_level, mask_number)
        all_qr.append(QR_code)
    qr_scores = [qr_score(qr) for qr in all_qr]
...
def main():
    ...
    best_qr = create_qr_with_best_mask(resulting_data, version, correction_level)
    save_QR_code(best_qr, 500)
